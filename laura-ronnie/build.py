...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .opt{display:flex; align-items:center; gap:10px; min-height:44px;
       padding:6px 0; margin-top:2px; cursor:pointer;}
  ...
  async function open(passcode, quiet) {
    ...
    try {
      ...
      if (!e.gone && lower !== passcode) {
        try {
          html = await unlockText("locked/page.enc", lower);
          used = lower;
          failure = null;
        } catch (e2) {
          failure = e2;
    ...
    player("entrance", "locked/laura-entrance.enc", used);
    player("cake", "locked/cake.enc", used);
  }
