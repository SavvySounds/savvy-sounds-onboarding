...
NEEDLES = [
    ...
    needle("script/store.gs", '      if (wanted.length < 8 || /\\s/.test(wanted)) return {ok: false, error: "invalid"};', '      if (false) return {ok: false, error: "invalid"};', "doors.test.mjs — Miles can change his pass to his own words, and only Miles", "a one-letter pass, or one with spaces, is accepted"),
    needle("script/store.gs", "    record.setTrashed(true);\n    if (history) history.setTrashed(true);", "    if (history) history.setTrashed(true);", "doors.test.mjs — Miles can remove a booking: its files go to the trash and its links close", "removing a booking closes its links and leaves the record where it was"),
    needle("script/store.gs", '        moment = {moment_id: bits[1], kind: "custom", label: "", date: null,', '        moment = {moment_id: bits[1], kind: "custom", label: bits[1], date: null,', "doors.test.mjs \u2014 a part of the night with no name of its own is still named in words", "a new part of the night is named after its own file id"),
    needle("script/store.gs", "        return who && who === actor;", "        return false;", "doors.test.mjs \u2014 nobody is asked to settle an answer against themselves", "one person's two saves make them argue with themselves"),
    needle("script/rules.gs", "    const was = _window(old);\n    if (!was) continue;", "    const was = _window(old);", "rules.test.mjs \u2014 a time written down for the first time has not moved", "writing a time down for the first time sends everybody back to re-confirm it"),
    # daysheet.gs: 3 guards
    needle("script/daysheet.gs", '      label: moment.label || _sheet_kind_words(moment.kind), room: moment.room || "", minutes: span_min(moment),', '      label: moment.label || moment.kind, room: moment.room || "", minutes: span_min(moment),', "doors.test.mjs \u2014 the day sheet prints the revision the zone and the cue words", "the day sheet calls an unnamed part of the night by the store's key"),
